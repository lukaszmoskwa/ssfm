...
class AskFileSaveDialog(CursBaseDialog):
    def fileSave(self):
        ...
        while self.enterKey != True:
            if self.focus == 0:
                self.win.addstr(9, pos_x[0], 'Save as',
                                self.focus_attr | self.opt_attr)
            else:
                self.win.addstr(9, pos_x[0], 'Save as', self.opt_attr)

            if self.focus == 1:
                self.win.addstr(9, pos_x[1], ' save  ',
                                self.focus_attr | self.opt_attr)
            else:
                self.win.addstr(9, pos_x[1], ' Save  ',   self.opt_attr)

            if self.focus == 2:
                self.win.addstr(9, pos_x[2], 'Cancel ',
                                self.focus_attr | self.opt_attr)
...
class ShowMessageDialog(CursBaseDialog):
    def showMessage(self):
        ...
        for (i, msg) in enumerate(self.message.split("\n")):
            self.win.addstr(i+1, 2, msg, self.msg_attr)
        rectangle(self.win, 8, int(self.x/2-2), 2,
                  3, self.opt_attr | self.focus_attr)
        self.win.addstr(9, int(self.x/2-1), 'Ok',
...
class ProgressBarDialog(CursBaseDialog):
    ...
    def displayMessage(self):
        if self.title:
            self.win.addstr(0, int(self.x/2-len(self.title)/2),
                            self.title, curses.A_BOLD | curses.A_STANDOUT)
        for (i, msg) in enumerate(self.message.split('\n')):
    ...
    def progress(self, currentValue):
        ...
        self.win.addstr(9, int(self.x/2-len(maxValue))-2,
...
if __name__ == '__main__':
    ...
    try:
        ...
        askYesCancelDialog(message='Ask Yes Cancel \njust for test',
                           title='Ask Yes Cancel Dialog', title_attr=curses.A_STANDOUT | curses.A_BOLD)
        showMessageDialog(message='Display message for test ',
                          title='Display message ')

        maxValue = 100
        progress = progressBarDialog(maxValue=maxValue, message='Progressbar for test',
                                     title='Progress test', clr1=COLOR_RED, clr2=COLOR_GREEN)
